...
spectrums = [s for s in load_from_mgf('save/casmi_2014_challenge_test.mgf')]

sirius_path = "comparison/casmi2014/SIRIUS"
sirius_files = [name for name in os.listdir(sirius_path) if os.path.isdir(os.path.join(sirius_path, name))]
sirius_index = [int(i.split('_')[-2].replace('Challenge', '')) for i in sirius_files]

deepmass_path = "D:/DeepMASS2_GUI/result/casmi2014"
deepmass_files = [name for name in os.listdir(deepmass_path)]
deepmass_index = [int(i.split('.')[0].replace('Challenge', '')) for i in deepmass_files]
